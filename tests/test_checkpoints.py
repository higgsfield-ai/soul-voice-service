...
def engine(monkeypatch, adapter, tmp_path):
    ...
    engine = VoiceEngine(Settings(checkpoints_dir=tmp_path / "checkpoints"))
...
def test_alternating_checkpoints_reuse_consumers_and_preserve_sampling(engine, payload, tmp_path, mode):
    ...
    for checkpoint in ("round1", "base", "raft", "round1", "raft", "base"):
        ...
        meta = engine.render(Payload.model_validate(payload), reference, tmp_path / "out.wav")
    ...
    meta = engine.render(Payload.model_validate(payload), reference, tmp_path / "default.wav")
